...
class zzb {
    ...
    private PendingIntent zzC(Bundle bundle) {
        ...
        for (String str : bundle2.keySet()) {
            if (str.startsWith(PushConstants.GCM_N) || str.startsWith("gcm.notification.")) {
                intent.removeExtra(str);
            }
        }
        return PendingIntent.getActivity(this.mContext, zzyj(), intent, PageTransition.CLIENT_REDIRECT);
    }

    private PendingIntent zzD(Bundle bundle) {
        Intent intent = new Intent("com.google.android.gms.gcm.NOTIFICATION_DISMISS");
        zza(intent, bundle);
        return PendingIntent.getService(this.mContext, zzyj(), intent, PageTransition.CLIENT_REDIRECT);
    }
    ...
    private String zzdV(String str) {
        return str.substring(PushConstants.GCM_N.length());
    }
    ...
    private int zzdW(String str) {
        ...
        if (!TextUtils.isEmpty(str)) {
            ...
            identifier = resources.getIdentifier(str, PushConstants.DRAWABLE, this.mContext.getPackageName());
            if (identifier != 0) {
                return identifier;
            }
    ...
    private Uri zzdX(String str) {
        return TextUtils.isEmpty(str) ? null : (PushConstants.SOUND_DEFAULT.equals(str) || this.mContext.getResources().getIdentifier(str, "raw", this.mContext.getPackageName()) == 0) ? RingtoneManager.getDefaultUri(2) : Uri.parse("android.resource://" + this.mContext.getPackageName() + "/raw/" + str);
    }
    ...
    static void zzz(Bundle bundle) {
        ...
        while (it.hasNext()) {
            ...
            if (str.startsWith(PushConstants.GCM_N)) {
                bundle2.putString(str.substring(PushConstants.GCM_N.length()), bundle.getString(str));
                it.remove();
