...
public class SplashScreen extends CordovaPlugin {
    ...
    class C02873 implements Runnable {
        ...
        /* renamed from: org.apache.cordova.splashscreen.SplashScreen.3.1 */
        class C02861 implements AnimationListener {
            C02861() {
            }
        ...
        public void run() {
            if (SplashScreen.splashDialog != null && SplashScreen.splashDialog.isShowing()) {
                ...
                fadeOut.setAnimationListener(new C02861());
            }
        }
    }
    ...
    class C02894 implements Runnable {
        ...
        public void run() {
            ...
            if ((SplashScreen.this.cordova.getActivity().getWindow().getAttributes().flags & WebInputEventModifier.NumLockOn) == WebInputEventModifier.NumLockOn) {
                SplashScreen.splashDialog.getWindow().setFlags(WebInputEventModifier.NumLockOn, WebInputEventModifier.NumLockOn);
            }
            ...
            if (this.val$hideAfterDelay) {
                new Handler().postDelayed(new C02881(), (long) this.val$effectiveSplashDuration);
            }
        }
    }
    ...
    class C02915 implements Runnable {
        ...
        public void run() {
            ...
            SplashScreen.spinnerDialog.setOnCancelListener(new C02901());
        ...
        if (!HAS_BUILT_IN_SPLASH_SCREEN) {
            ...
            if (this.preferences.getInteger("SplashDrawableId", 0) == 0) {
                ...
                if (splashResource != null) {
                    int drawableId = this.cordova.getActivity().getResources().getIdentifier(splashResource, PushConstants.DRAWABLE, this.cordova.getActivity().getClass().getPackage().getName());
                    if (drawableId == 0) {
                        drawableId = this.cordova.getActivity().getResources().getIdentifier(splashResource, PushConstants.DRAWABLE, this.cordova.getActivity().getPackageName());
                    }
    ...
    private int getFadeDuration() {
        ...
        if (fadeSplashScreenDuration < 30) {
            return fadeSplashScreenDuration * PointerIconCompat.STYLE_DEFAULT;
        }
    ...
    public boolean execute(String action, JSONArray args, CallbackContext callbackContext) throws JSONException {
        if (action.equals("hide")) {
            this.cordova.getActivity().runOnUiThread(new C02841());
        } else if (!action.equals("show")) {
            return HAS_BUILT_IN_SPLASH_SCREEN;
        } else {
            this.cordova.getActivity().runOnUiThread(new C02852());
        }
    ...
    private void removeSplashScreen(boolean forceHideImmediately) {
        this.cordova.getActivity().runOnUiThread(new C02873(forceHideImmediately));
    }
    ...
    private void showSplashScreen(boolean hideAfterDelay) {
        ...
        if (splashscreenTime > 0 || !hideAfterDelay) {
            this.cordova.getActivity().runOnUiThread(new C02894(drawableId, hideAfterDelay, effectiveSplashDuration));
        }
    }

    private void spinnerStart() {
        this.cordova.getActivity().runOnUiThread(new C02915());
    }

    private void spinnerStop() {
        this.cordova.getActivity().runOnUiThread(new C02926());
    }
}
